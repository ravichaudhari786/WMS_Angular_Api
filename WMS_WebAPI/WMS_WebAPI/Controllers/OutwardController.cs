...
namespace WMS_WebAPI.Controllers
{
    public class OutwardController : ApiController
    {
        ...
        [HttpGet]
        [Route("api/Outward/OutwardReceipt")]
        public IHttpActionResult OutwardReceipt(int OutwardID,int WarehouseID)
        {
            try
            {
                ...
                using (SqlConnection connection = new SqlConnection(connectionstring))
                {
                    using (SqlCommand command = new SqlCommand("Rep_GatePass", connection))
                    {
                        command.CommandType = System.Data.CommandType.StoredProcedure;
                        SqlParameter[] param = new SqlParameter[2];
                        param[0] = new SqlParameter("@OutwardID", OutwardID);
                        ...
                        {
                            da.Fill(ds);
                            DevExpress.XtraReports.UI.XtraReport report = GetSelectedReport("Material Outward Receipt");
                            if (report != null)
                            {
