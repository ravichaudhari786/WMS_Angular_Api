...
namespace WMS_WebAPI.Controllers
{
    ...
    public class InwardController : ApiController
    {
        ...
        [HttpGet]
        [Route("api/Inward/InwardReceipt")]
        public IHttpActionResult InwardReceipt(int InwardID)
        {
            try
            {
                ...
                using (SqlConnection connection = new SqlConnection(connectionstring))
                {
                    using (SqlCommand command = new SqlCommand("Rep_MaterialReceipt", connection))
                    {
                        command.CommandType = System.Data.CommandType.StoredProcedure;
                        SqlParameter[] param = new SqlParameter[1];
                        param[0] = new SqlParameter("@InwardID", InwardID);
                        command.Parameters.AddRange(param);
                        connection.Open();
                        using (SqlDataAdapter da = new SqlDataAdapter(command))
                        {
                            da.Fill(ds);
                            DevExpress.XtraReports.UI.XtraReport report = GetSelectedReport("Material Inward Receipt");
                            if (report != null)
                            {
